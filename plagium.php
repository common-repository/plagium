...
class Plagium {
    ...
    public function admin_options(){
	    ...
	    if(isset($_POST['plagium_ispost']) && $_POST['plagium_ispost'] == 'true') {
	          //Form data sent
	    ...
	    <div class="wrap">
	      ...
	      <h2><?php _e( 'Plagium', self::LANGUAGE ); ?></h2>
    ...
    ?>
		      ...
		      <form name="plagium_settings_form" id="plagium_settings_form" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
			    ...
			    ?>
		        <p class="submit"><input type="button" class="button-primary" id="plagium_settings_submit" name="Submit" value="<?php _e('Save Changes', self::LANGUAGE ) ?>" /></p>
		      </form>
	      </div>
