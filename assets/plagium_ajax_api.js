...
plagiumInvalidApiKey = function () {
    ...
    if (path.indexOf('wp-admin/') >= 0) {
        path = path.substring(0, path.indexOf('wp-admin/') + "wp-admin/".length);
    }
    else path = "";
    jQuery('#plagium_error').html('Invalid Plagium Account Information!.<br >Please, check your Plagium Plugin Settings <a href="'+path+'options-general.php?page=plagium-admin">here</a>.');
}
